...
GLOBALDEF int ftplib_debug = 3; // nibaozhu
#endif
...
void * memccpy(char *dest, const char *src, int c, size_t n)
{
    int i=0;
...
// add 2013-09-20
char* my_memccpy(char *dest, const char *src, char c, unsigned int  n)
{
    unsigned int i=0;
    const char *ip = src;
    char *op = dest;
...
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl)
{
	...
	*pnum++ = '\0';
    ...
    if ((int)(sin.sin_addr.s_addr = inet_addr(lhost)) == -1) //if ((sin.sin_addr.s_addr = inet_addr(lhost)) == -1) //modify 2013-09-20
    {
    ...
    // ctrl = calloc(1,sizeof(netbuf)); //modify 2013-09-20 lmt
    ...
    //ctrl->buf = malloc(FTPLIB_BUFSIZ); //modify 2013-09-20 lmt
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir)
{
    ...
    if ((mode == 'A') && ((ctrl->buf = (char *) malloc(FTPLIB_BUFSIZ)) == NULL)) // ctrl->buf = malloc(FTPLIB_BUFSIZ)  modify 2013-09-20 lmt
    {
	perror("calloc");
	net_close(sData);
	free(ctrl);
...
GLOBALDEF int FtpRead(void *buf, int max, netbuf *nData)
{
	...
	return 0;
    if (nData->buf)
        i = readline((char *)buf, max, nData); // i = readline(buf, max, nData); // modify 2013-09-20 lmt
    else
    {
        i = socket_wait(nData);
	if (i != 1)
...
GLOBALDEF int FtpWrite(void *buf, int len, netbuf *nData)
{
	...
	return 0;
    if (nData->buf)
    	i = writeline((char *)buf, len, nData); // i = writeline(buf, len, nData); modify 2013-09-20 lmt
    else
    {
        socket_wait(nData);
#ifndef _MSC_VER
	...
	netbuf *nControl, int typ, int mode)
{
	...
	return 0;
    //dbuf = malloc(FTPLIB_BUFSIZ); //modify 2013-09-20 lmt
