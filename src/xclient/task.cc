...
int init(int argc, char **argv, std::map<int, Transport*> *m) {
	...
	do {
		...
		ev.data.fd = connect_sock; /* connect's fd */
		...
		t = new Transport(connect_sock, created, addr, addrlen);
		m->insert(std::make_pair(connect_sock, t));

		plog(info, "Epoll instance file = %d.\n", epollfd);
		plog(info, "Refer to by sockfd = %d the address specified by addr = {%s:%d}.\n", connect_sock, ip, port);
	} while (false);
	return ret;
}
...
int uninit(std::list<Transport*> *r, std::list<Transport*> *w, std::map<int, Transport*> *m, std::map<uint32_t, int> *__m) {
	...
	do {
		...
		if (connect_sock > 0) {
			plog(info, "Close active file = %d.\n", connect_sock);
			ret = close(connect_sock);
			if (ret == -1) {
				plog(error, "%s(%d)\n", strerror(errno), errno);
			}
		}
...
#if 0
				int acceptfd = accept(connect_sock, (struct sockaddr *) &peer_addr, &peer_addrlen);
				if (acceptfd == -1) {
					plog(error, "%s(%d)\n", strerror(errno), errno);
					break;
				}
